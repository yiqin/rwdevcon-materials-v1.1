...
class DetailsInterfaceController: WKInterfaceController {
  ...
  func toggleFavorite() {
    if let session = session {
      ...
      configureContextMenu()
      WKInterfaceController.openParentApplication(["identifier": session.identifier, "isFavorite": session.isFavorite], reply: { (replyInfo, error) -> Void in
        if let success = replyInfo["success"] as? Bool {
          println("Favorite status for '\(session.title)' synced with iOS app: \(success)")
        }
      })
    }
  }
